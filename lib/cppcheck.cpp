...
unsigned int CppCheck::check (const std::string &path)
{
	if (!mParent) {
		...
		mChildren.back ()->mSettings = mSettings;
	}

	CppCheck &temp = !mParent ? *mChildren.back () : *this;
	...
	return temp.checkFile (Path::simplifyPath (path), emptyString, fin);
}
	...
	if (!Settings::terminated ()) {
		if (!mChildren.empty ()) {
			for (auto child : mChildren) {
				if (child->mTokenizer && child->mTokenizer->getSymbolDatabase ()) {
			...
			Timer timerRunChecks (
				check.name () + "::runChecks", mSettings.showtime, &s_timerResults);

			check.runChecks (&*mTokenizer, &mSettings, this, &mImplementations);
		}
	}
}
...
void CppCheck::registerImplementations () {
	...
	for (const Scope *scope : symbols->functionScopes) {
		...
		if (scope->function && scope->function->tokenDef->fileIndex () > 0) {
			CheckExceptionSafety::FunctionImpls &impls =
				mParent ? mParent->mImplementations : mImplementations;

			impls[scope->function->getQualifiedName ()] = std::make_pair (scope, this);
		}
	}
}