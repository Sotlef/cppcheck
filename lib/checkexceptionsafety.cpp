...
static const Token *functionThrowsRecursive (const Function *function, std::set<const Function *> &recursive, const CheckExceptionSafety::FunctionImpls *impls)
{
	...
	if (!function->functionScope && impls) {
		auto name = function->getQualifiedName ();
		auto it = impls->find (name);

		if (it != impls->end ()) {
			if (name == it->second.first->function->getQualifiedName ())
				function = it->second.first->function;
		}

		if (!function || !function->functionScope)
			return nullptr;
	}
		 ...
		 tok != function->functionScope->bodyEnd; tok = tok->next ())
	{
		...
		if (tok->str () == "new") {
			...
			// check if called functionhas an exception specification
			...
			else if (!called->isNoExcept ()) {
				if (functionThrowsRecursive (called, recursive, impls)) {
					return tok;
				}
			}
		}
		else if (tok->str () == "throwException") {
			return tok;
		}
	}
